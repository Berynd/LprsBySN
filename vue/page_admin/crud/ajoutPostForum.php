...
require_once "../../../src/repository/PostForumRepository.php";
require_once "../../../src/modele/PostForum.php";
...
$repCategorie = new CategorieForumRepository();
$nbCategorie = $repCategorie->nombreCategorieForum();

$post = new PostForumRepository();
$categorie = $post->getCategorieForum();
$utilisateur = $post->getUtilisateur();
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<main class="main">
    ...
    <div class="content">
        <h2 class="form-title">➕ Ajouter un post</h2>
        <form method="POST" action="../../../src/traitement/CategorieForum/TraitementAjoutCategorieForum.php" class="add-form">
            ...
            <div class="form-group">
                ...
                <input type="text" id="contenue" name="contenue" required>
            </div>
            <div class="form-group">
                <label for="date_post">Date du post :</label>
                <input type="datetime-local" id="date_post" name="date_post" required>
            </div>
            <div class="form-group">
                <label for="refCategorieForum">Categorie du forum : </label>
                <select id="refCategorieForum" name="refCategorieForum" required>
                    <?php foreach ($categorie as $categorie) {
                        echo "<option value='" . $categorie["id_categorie_forum"] . "'>" . $categorie["nom"] ."</option>";
                    } ?>
                </select>
            </div>
            <div class="form-group">
                <label for="refUtilisateur">Utilisateur </label>
                <select id="refUtilisateur" name="refUtilisateur">
                    <?php foreach ($utilisateur as $utilisateur) {
                        echo "<option value='" . $utilisateur["id_utilisateur"] . "'>" . $utilisateur["nom"] . "</option>";
                    } ?>
                </select>
            </div>
            <button type="submit" class="btn-submit">Ajouter le post</button>
        </form>
    </div>
